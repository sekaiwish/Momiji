...
async def dump(ctx):
    try:
        #await bot.say(str(ctx.message.channel.name))
        if ctx.message.author.discriminator == "6151":
            i = 0

            try:
                c = bot.logs_from(ctx.message.channel, limit=int(ctx.message.content.split(' ')[1]))
                print(int(ctx.message.content.split(' ')[1]))
            except:
                c = bot.logs_from(ctx.message.channel, limit=10)

            if not os.path.exists("logs"):
                os.makedirs("logs")

            filename = "logs\\" + "discLogs_" + ctx.message.channel.name + ".txt"
            ...
            #await bot.say("done: " + str(i) + " pictures dumped to txt in: " + "\\" + "logs\\" + "discLogs_" + ctx.message.channel.name + ".txt")
            await bot.say("downloading...")
            downloadTxt(filename, ctx.message.channel.name)
            #await bot.say("done downloading to \\" + ctx.message.channel.name)
...
async def dump_text(ctx):
    try:
        ...
        if ctx.message.author.discriminator == "6151":
            ...
            filename = "logs\\" + "discLogs_Text_" + ctx.message.channel.name + ".txt"
...
@bot.command(pass_context=True)
async def grant(ctx):
    if ctx.message.author.discriminator == "9377":
...
async def rl(ctx):
    ...
    try:
        ...
        CC_add = "logs\\" + "discLogs_Text_" + ctx.message.channel.name + ".txt"
        ccF = open(CC_add, 'r')
...
def rl_inBot(ctx, twoLine = False):
    try:
        CC_add = "logs\\" + "discLogs_Text_" + ctx.message.channel.name + ".txt"
        ccF = open(CC_add, 'r')
        CC = ccF.readlines()  # global
...
async def rt(ctx):
    ...
    try:
        lengthIn = int(ctx.message.content.split(' ')[1])
        if lengthIn > 2000:
            limit=2000
    except:
        lengthIn=200
...
async def ri(ctx):
    ...
    folderPath = ctx.message.channel.name
    ...
    for fileIt in files:
        path = Path(fileIt)
        if path.exists():
            await bot.send_file(ctx.message.channel, fileIt, content=rl_inBot(ctx))
        else:
...
async def rp(ctx):
    ...
    names = ["bean","joey","jacob","nick","godfrey","steven","derek","kyle tamondong","roman","ben cimperman","anna","grant","jimmy","cole","max","dylan","kash","ian","will",
             "skyler","kevin","charlie","ayush","derek","sam","asstumpy","jackson","jackson koenig","matt","sammy","taylor","oneassen","mumble mom","max haland","jon","mike warner","bartleby","himsel","dion a", "one half assen", "johnny", "johnny silktouch", "martin smith","charles terzian","justin","vishnu","hunter","murder black teen","nick barone","kevin roland", "dennis", "sebastian", "neetorin", "cole", "brad", "texas cole", "fourchan", "ran", "ryne", "colten", "nebuchadnezzar","cody", "wills parents" ]
    text1 = rl_inBot(ctx)
    text2 = rl_inBot(ctx)
...
@bot.command(pass_context=True)
async def jacob(ctx):
    if ctx.message.author.discriminator == "9393":
...
@bot.command(pass_context=True)
async def steveo(ctx):
    if ctx.message.author.discriminator == "0001":
...
@bot.command(pass_context=True)
async def kash(ctx):
    if ctx.message.author.discriminator == "0770":
...
@bot.command(pass_context=True)
async def ayush(ctx):
    if ctx.message.author.discriminator == "7268":
...
async def setP(ctx):
    if ctx.message.author.discriminator == "6151":
        await bot.change_presence(game=discord.Game(name=ctx.message.content))
